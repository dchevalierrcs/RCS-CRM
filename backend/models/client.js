...
/* ==============================================================
 * GET /api/clients – liste avec jointures
 * ============================================================== */
router.get('/', async (_req, res) => {
  try {
    const sql = `
      ...
      GROUP BY c.id, g.nom, cr.id, pp.id, rtm.nom, p.code_iso
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const mainQuery = `
      SELECT
        c.*, g.nom as groupement_nom, p.code_iso,
        rtm.nom as type_marche, pp.nb_departs_pub, pp.nb_webradios,
    ...
    const [contactsRes, audiencesRes, servicesRes] = await Promise.all([
      pool.query(contactsQuery, [clientId]),
      pool.query(audiencesQuery, [clientId]),
      pool.query('SELECT cs.id, cs.service_id, cs.description, cs.valeur_mensuelle, rs.nom, rs.categorie FROM client_services cs JOIN ref_services rs ON cs.service_id = rs.id WHERE cs.client_id = $1 ORDER BY rs.categorie, rs.nom', [clientId])
    ]);
  ...
  try {
    ...
    const clientInsertResult = await client.query(
      ...
      [
        ...
        clientData.revenus_programmation_mensuel, clientData.revenus_diffusion_mensuel, clientData.revenus_planification_mensuel, clientData.revenus_streaming_mensuel
      ]
    );
  ...
  try {
    ...
    await client.query(
      `UPDATE clients SET
       ...
       WHERE id=$13`,
      [
        clientData.nom_radio.toUpperCase(),
        clientData.nom_groupe?.toUpperCase(),
        clientData.raison_sociale?.toUpperCase(),
        clientData.adresse,
        clientData.pays,
        clientData.statut_client,
        clientData.rcs_id,
        clientData.groupement_id || null,
        clientData.revenus_programmation_mensuel,
        clientData.revenus_diffusion_mensuel,
        clientData.revenus_planification_mensuel,
        clientData.revenus_streaming_mensuel,
        clientId
      ]
    );
